...
# ---- 工况参数 ----
# ⚠️ 2026-09-24 口径变化：100×100 与 200×200 两档的子步数由 20 / 8 提到 **100**。
#    原因是"组装+分解"是**一次性**成本（只在 stamp 变化时发生），脚本按"累计 ÷ 子步数"折算，
#    子步数越少它被摊得越大：200×200 用 8 子步时，单子步 49.2 ms 里有 **43 %** 是这一项
#    （实测：8 子步 49.224 ms vs 100 子步 28.541 ms），而 100×100 用 20 子步也偏大约 26 %。
#    ⇒ 曲线与旧日志**不再逐字可比**（旧日志已被本次重采覆盖）；引用历史绝对值时注意这条。
# ⚠️ **1T / 2T 两档在本机上是"双峰"的**（同一二进制同参数两次可差 ~1.7×：1T/200×200 实测
#    123.6–138.6 ms，加 PD_AFFINITY=workers 钉核后 72.9–76.7 ms）⇒ 这两档别用来下并发结论。
#    仍按默认采（保持与多线程档同一配置）；需要可信的 1T/2T 时单独用 PD_AFFINITY=workers 跑并注明。
$workloads = @(
  ...
  @{ id = '100x100_it10'; args = @('--grid', '100', '100', '--steps', '100', '--iters', '10', '--residual-tol', '0.001') },
  @{ id = '200x200_it10'; args = @('--grid', '200', '200', '--steps', '100', '--iters', '10', '--residual-tol', '0.001') }
)
