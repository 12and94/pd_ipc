...
<div class="card">
  ...
  <div class="sub" style="margin:0 0 10px">
    ...
    这里按"累计 ÷ 子步数"折算，因此子步数少的工况里它会明显偏大。
    ✅ 2026-09-24 起 <b>100×100 与 200×200 这两档的子步数已由 20 / 8 提到 100</b>（旧口径下
    200×200 的"单子步"里有 <b>43 %</b> 是这一项：8 子步 49.2 ms vs 100 子步 28.5 ms）——
    这是**口径变化**，与旧日志不再逐字可比。
  </div>
...
<div class="card"><div class="sub" style="margin:0 0 10px">
  ...
  ⚠️ <b>1T / 2T 两档不可信</b>（2026-09-24 发现）：它们在本机上是<b>双峰</b>的 —— 同一二进制、
...
<div class="card">
  <div class="sub" style="margin:0 0 10px">
    ...
    · <b>只加弯曲</b>：代价几乎全进 <code>全局回代</code>（弯曲没有局部步/散射）——
      <b>但要按规模分开读</b>：40×40/60×60 上回代涨 2.0×/2.4×，而 <code>投影+散射</code> 几乎不动（0.99×/0.97×）；
      到了 100×100 回代涨 <b>3.05×</b>、<code>投影+散射</code> 也跟着涨 1.20×（整条迭代的访存压力都变大了）；<br>
    · <b>两者都开</b>：不是简单相加 —— 100×100 上它比"只加弯曲"<b>还快</b>（15.4 vs 16.8 ms/子步），
      原因见下表脚注（<b>填充不是约束数的单调函数</b>）。<br>
    ⚠️ <b>这些比值取决于二进制版本</b>：2026-09-24「因子只存一份」上线后，<b>因子更大</b>的矩阵
    （含弯曲的那种）从该优化里受益**更多** —— 100×100"只加弯曲"的单子步 26.0 → 16.8 ms（−35 %），
    而默认集只 −1.5 % ⇒ 本节的比值整体<b>变小</b>（回代 5.9× → 3.05×）。
    引用绝对值时请连采集时间一起引（每节标题下方有）。<br>
  ...
  for (const w of CS) {
    ...
    if (bendOnly && both) {
      ...
      t += '<div class="sub" style="margin-top:8px"><b>⚠️ 反直觉但可复现</b>：'
         ...
         + '这既是"多一类约束不一定更慢"的例子，也提示"给含弯曲的矩阵找一个更好的排序"还有空间（2026-09-24 已实测结案：内置排序 AMD 完胜、「只排一条链再复制」比值恰好 1.0000 ⇒ 此路不通，见 docs/perf.md §17）'
