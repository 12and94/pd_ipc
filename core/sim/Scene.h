...
struct SceneConfig {
  ...
  /// 等效抗弯刚度与网格间距**强相关**。**具体幂次是实测出来的**（`pd_bendaudit` 的 H 节：
  /// 给定形状直接算能量、不跑迭代）：二维板 `D_eff = k·s²·N(N−2)/(N−1)² ∝ k·s²`，
  /// 一维纤维 `∝ k·s³`，对 k 严格线性。
  /// 2026-09-23 修正：本注释原先写的是"∝ s⁴ / k"，**方向与幂次都不对**（k 大了当然更硬）。
  /// **工程含义**：换间距/分辨率必须重新标定，而且要**按 `k ∝ 1/s²` 调**
  /// —— 网格越细，同一个 k 越软，必须调大 k。要尺度无关得改成余切权重的等距弯曲
  /// （`Q = KᵀMK`）或按质量参数化（与 κ 的处理原则一致）。**这一步没有做。**
...
//   `accurate`  —— 离线对照 / 回归：80 次 / 1e-3
//
// ⚠️ **门槛（`residualTolerance`）只在"接近平衡"时才起作用**（2026-09-24 实测，`docs/perf.md` §18.6）：
//   查看器稳态（顶边整条 pin、κ=2305）的残差就停在 **0.29·|g|** 附近 ⇒ 0.3 门槛**1 次迭代**即满足
//   （这正是当初选 0.3 的原因：1e-1 要 40 次迭代）；但把它换成 1e-3 后**跑满 80 次也才勉强压到
//   0.0094·|g|**。而在"一直摆/刚度大"的工况（只 pin 两角、或 κ=1e4）残差常在 0.4–7.7·|g|，
//   **门槛永不触发**，这时**只有迭代上限在起作用** ⇒ 三档的真实差别就是 20/40/80 次迭代。
//   所以调门槛**不是**调精度的旋钮；要更接近平衡只能加迭代数（`scene` 的性质，不是档位的属性）。
//
