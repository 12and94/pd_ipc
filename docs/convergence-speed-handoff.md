...
|---|---|---|---|---|
| 100×100（10 000 顶点）κ=2305、h=1/120、顶边 pin | 瞬态（第 60 子步） | 2 次 | 7 次 | 0.86 |
| 同上 | 稳态（第 600 子步） | 未达到 | 未达到（48 次内） | 0.9977 |
| 60×60 | 瞬态 / 稳态 | 4 次 / 未达到 | 17–18 次 / 未达到 | 0.83 / 0.9975 |
...
```powershell
$env:PD_SHEAR='3000000'    # 对角（剪切）刚度 3e6
...
质量换算（实测，非 `ρ·s²`）：本引擎每顶点质量 = **1.95 · ρ · s²**；故 `--density 5128` 对应 m = 1。
