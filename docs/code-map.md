...
| `_verify/share_factor.cpp` | **三分量因子"只存一份 vs 各存一份"的实测**（`pd_sharefactor`）：① 前提自检（把参考分量压成紧凑单份，逐列核对另两份"同构 + 数值逐位相同"，含对角 `1/D`）；② 单份求解 vs 各份求解**逐位比对**；③ 同一并行区域内交替测 A/B，**冷/热两档**（冷档每轮清 64 MB 缓存）；④ 一次性构建成本。结论见 `docs/perf.md` §16（**只测量，未做生产改造**） |
