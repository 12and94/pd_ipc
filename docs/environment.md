...
### 5.1 【铁律】禁止参考任何外部同类实现
本项目**不允许**以任何形式参考、借用、搬运或对照任何外部同类实现（本机工作区中其他目录下的项目一律在内），包括但不限于：

- 不读其源码（求解器、碰撞、约束、网格、渲染等任何模块）；
- 不采用其方案、推导、默认参数或"已验证结论"；
- 不引用其文档、调试记录或测试用例；
- 不在其他项目的工作目录下构建或运行任何东西；
- 不在本项目的代码、注释、文档、提交信息中出现对其他工程路径的引用或依赖。

**理由**：本项目要求从标准 PD 公式与文献出发、独立推导并逐项验证。任何以他人实现为对照的做法，都会把对方的隐含假设（无论对错）带进来，并让"验证"退化为"复现"。本项目的正确性来源只能是**数学推导 + 原语不变量断言 + 与解析解的对照**（见 `docs/plan.md` §2.2、M1）。

**执行方式**：规定写进 `docs/contributing.md`，并由 CI 检查（扫描非白名单文件中对本工作区其他工程的路径引用，命中即失败）；构建配置一律使用本项目内的相对路径，不依赖任何其他工程目录。
...
| 渲染可用 GLFW | 本地有 GLFW 3.3.8 (vc2022) | M3 可直接用 GLFW；渲染器与 UI 在本项目内独立实现（见 §5.1 禁用规定）；ImGui 需自行获取 |
